...
class PlaneswalkerStartingLoyaltyTextFieldDelegate: NSObject, NSControlTextEditingDelegate {
  override func controlTextDidChange(_ obj: Notification) {
    guard let doc = NSDocumentController.shared.currentDocument as? Document,
      let card = doc.card else {
        return
    }
    card.planeswalkerStartingLoyalty = doc.startingLoyaltyTextField.stringValue
