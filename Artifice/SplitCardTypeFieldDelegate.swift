...
class SplitCardTypeFieldDelegate: NSObject, NSControlTextEditingDelegate {
  override func controlTextDidChange(_ obj: Notification) {
    ...
    child.cardType = doc.splitCardRightCardTypeTextField.stringValue
    doc.landscapeCardView.activeTemplate?.updateTypeLine(card: card)
    doc.landscapeCardView.activeTemplate?.updateTextBox(card: card)
    doc.landscapeCardView.activeTemplate?.updateFrameBorder(card: card)
    doc.landscapeCardView.activeTemplate?.updateBackgroundTexture(card: card)
    
    if let row = doc.searchResults.index(of: card) {
      let rowIndex = IndexSet(integer: row)
      let columnIndex = IndexSet(integer: 2)
