...
class StandardTemplateCardView: CardTemplateView {
  
  struct BackgroundTexture {
    static var White = NSImage(named: NSImage.Name(rawValue: "Standard White Background Texture"))!
    static var Blue = NSImage(named: NSImage.Name(rawValue: "Standard Blue Background Texture"))!
    static var Black = NSImage(named: NSImage.Name(rawValue: "Standard Black Background Texture"))!
    static var Red = NSImage(named: NSImage.Name(rawValue: "Standard Red Background Texture"))!
    static var Green = NSImage(named: NSImage.Name(rawValue: "Standard Green Background Texture"))!
    static var Colorless = NSImage(named: NSImage.Name(rawValue: "Standard Artifact Background Texture"))!
    static var Multicolored = NSImage(named: NSImage.Name(rawValue: "Standard Multicolored Background Texture"))!
    static var Land = NSImage(named: NSImage.Name(rawValue: "Standard Land Background Texture"))!
    static var Vehicle = NSImage(named: NSImage.Name(rawValue: "Standard White Background Texture"))!
    struct Hybrid {
      struct White {
        static var Blue = NSImage(named: NSImage.Name(rawValue: "Standard White Blue Hybrid Background Texture"))!
        static var Black = NSImage(named: NSImage.Name(rawValue: "Standard White Black Hybrid Background Texture"))!
      }
      struct Blue {
        static var Black = NSImage(named: NSImage.Name(rawValue: "Standard Blue Black Hybrid Background Texture"))!
        static var Red = NSImage(named: NSImage.Name(rawValue: "Standard Blue Red Hybrid Background Texture"))!
      }
      struct Black {
        static var Red = NSImage(named: NSImage.Name(rawValue: "Standard Black Red Hybrid Background Texture"))!
        static var Green = NSImage(named: NSImage.Name(rawValue: "Standard Black Green Hybrid Background Texture"))!
      }
      struct Red {
        static var Green = NSImage(named: NSImage.Name(rawValue: "Standard Red Green Hybrid Background Texture"))!
        static var White = NSImage(named: NSImage.Name(rawValue: "Standard Red White Hybrid Background Texture"))!
      }
      struct Green {
        static var White = NSImage(named: NSImage.Name(rawValue: "Standard Green White Hybrid Background Texture"))!
        static var Blue = NSImage(named: NSImage.Name(rawValue: "Standard Green Blue Hybrid Background Texture"))!
      }
    }
  }
  ...
  static var artSize = NSSize(width: 394, height: 296)
  static var artAspectRatio: CGFloat = 394 / 296
  ...
  func initSubviews() {
    Bundle(for: type(of: self))
      .loadNibNamed(NSNib.Name(rawValue: "StandardTemplateCardView"), owner: self, topLevelObjects: nil)
  ...
  override func updateBackgroundTexture(card: Card) {
    ...
    case 0...5 where card.isLand:
      backgroundTexture.image = BackgroundTexture.Land
    case 0...5 where card.isArtifact && card.isVehicle:
      backgroundTexture.image = BackgroundTexture.Vehicle
    case 0...5 where card.isArtifact:
      backgroundTexture.image = BackgroundTexture.Colorless
    case 3...5:
      backgroundTexture.image = BackgroundTexture.Multicolored
    case 2 where !card.isHybrid:
      backgroundTexture.image = BackgroundTexture.Multicolored
    case 2 where card.isWhite && card.isBlue:
      backgroundTexture.image = BackgroundTexture.Hybrid.White.Blue
    case 2 where card.isWhite && card.isBlack:
      backgroundTexture.image = BackgroundTexture.Hybrid.White.Black
    case 2 where card.isBlue && card.isBlack:
      backgroundTexture.image = BackgroundTexture.Hybrid.Blue.Black
    case 2 where card.isBlue && card.isRed:
      backgroundTexture.image = BackgroundTexture.Hybrid.Blue.Red
    case 2 where card.isBlack && card.isRed:
      backgroundTexture.image = BackgroundTexture.Hybrid.Black.Red
    case 2 where card.isBlack && card.isGreen:
      backgroundTexture.image = BackgroundTexture.Hybrid.Black.Green
    case 2 where card.isRed && card.isGreen:
      backgroundTexture.image = BackgroundTexture.Hybrid.Red.Green
    case 2 where card.isRed && card.isWhite:
      backgroundTexture.image = BackgroundTexture.Hybrid.Red.White
    case 2 where card.isGreen && card.isWhite:
      backgroundTexture.image = BackgroundTexture.Hybrid.Green.White
    case 2 where card.isGreen && card.isBlue:
      backgroundTexture.image = BackgroundTexture.Hybrid.Green.Blue
    case 1 where card.isWhite:
      backgroundTexture.image = BackgroundTexture.White
    case 1 where card.isBlue:
      backgroundTexture.image = BackgroundTexture.Blue
    case 1 where card.isBlack:
      backgroundTexture.image = BackgroundTexture.Black
    case 1 where card.isRed:
      backgroundTexture.image = BackgroundTexture.Red
    case 1 where card.isGreen:
      backgroundTexture.image = BackgroundTexture.Green
    default:
      backgroundTexture.image = BackgroundTexture.Colorless
    }
  }
  ...
  override func updateTitlebarAndTypeLine(card: Card) {
    switch card.colors {
    case 3...5:
      titlebarAndTypeLine.image = NSImage(named: NSImage.Name(rawValue: "Standard Multicolored Titlebar & Type Line"))
    case 2 where !card.isHybrid && !card.isLand:
      titlebarAndTypeLine.image = NSImage(named: NSImage.Name(rawValue: "Standard Multicolored Titlebar & Type Line"))
    case 2:
      titlebarAndTypeLine.image = NSImage(named: NSImage.Name(rawValue: "Standard Hybrid Titlebar & Type Line"))
    case 1 where card.isWhite:
      titlebarAndTypeLine.image = NSImage(named: NSImage.Name(rawValue: "Standard White Titlebar & Type Line"))
    case 1 where card.isBlue:
      titlebarAndTypeLine.image = NSImage(named: NSImage.Name(rawValue: "Standard Blue Titlebar & Type Line"))
    case 1 where card.isBlack:
      titlebarAndTypeLine.image = NSImage(named: NSImage.Name(rawValue: "Standard Black Titlebar & Type Line"))
    case 1 where card.isRed:
      titlebarAndTypeLine.image = NSImage(named: NSImage.Name(rawValue: "Standard Red Titlebar & Type Line"))
    case 1 where card.isGreen:
      titlebarAndTypeLine.image = NSImage(named: NSImage.Name(rawValue: "Standard Green Titlebar & Type Line"))
    case 0 where !card.isLand && card.isArtifact:
      titlebarAndTypeLine.image = NSImage(named: NSImage.Name(rawValue: "Standard Colorless Artifact Titlebar & Type Line"))
    case 0 where card.isLand:
      titlebarAndTypeLine.image = NSImage(named: NSImage.Name(rawValue: "Standard Colorless Land Titlebar & Type Line"))
    default:
      titlebarAndTypeLine.image = NSImage(named: NSImage.Name(rawValue: "Standard Colorless Titlebar & Type Line"))
    }
  }
  ...
  override func updateCardName(card: Card) {
    ...
    let cardNameAttributedString =
      NSMutableAttributedString(string: card.name,
                                attributes: cardNameAttributes)
    
    name.attributedStringValue = cardNameAttributedString
  }
  ...
  override func updateRulesText(card: Card) {
	...
	let rulesTextAttributes: [NSAttributedStringKey : Any] = [
  ...
  override func updateFlavorText(card: Card) {
    ...
    filterEmphasis(italic: false, string: flavorTextAttributedString, size: fontSize, align: flavorTextParagraphStyle.alignment)
